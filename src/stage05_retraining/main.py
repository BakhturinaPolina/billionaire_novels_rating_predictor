...
def retrain(pareto_csv: Path, top_n: int, config: Path, output_dir: Path):
    ...
    # Load configuration
    print(f"\n[RETRAIN_CMD] Loading configuration...")
    print(f"[RETRAIN_CMD]   Config file: {config}")
    ...
    dataset_csv = inputs.get("chapters_csv", "data/processed/chapters.csv")
    dataset_path = resolve_path(Path(dataset_csv))
    print(f"[RETRAIN_CMD]   Dataset CSV resolved: {dataset_path}")
