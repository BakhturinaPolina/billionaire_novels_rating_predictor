...
# Load config
try:
    config_path = Path("configs/labeling.yaml")
    if not config_path.exists():
        config_path = Path(__file__).parent.parent.parent / "configs" / "labeling.yaml"
    cfg = load_config(config_path)
    scoring_cfg = load_config(Path("configs/scoring.yaml"))
    
    PREP = resolve_path(Path(scoring_cfg["output"]["prepared_books_parquet"]))
    MANUAL_MD = resolve_path(Path(scoring_cfg["inputs"]["manual_mapping_md"]))
    CODEBOOK_CSV = resolve_path(Path(scoring_cfg["inputs"]["codebook_csv"])) if scoring_cfg["inputs"].get("codebook_csv") else None
    OUT_PARQUET = resolve_path(Path(cfg["output"]["composites"]))
except Exception as e:
    # Fallback to defaults if config loading fails
    print(f"Warning: Could not load config, using defaults: {e}")
    PREP = "data/processed/prepared_books.parquet"
    MANUAL_MD = "results/experiments/reproducible_scripts topics vs readers appreciation/Billionaire_Manual_Mapping_Topics_by_Thematic_Clusters.md"
    CODEBOOK_CSV = "results/experiments/reproducible_scripts topics vs readers appreciation/focused_topic_codebook.csv"
    OUT_PARQUET = "results/topics/ap_composites.parquet"
