...
try:
    ...
    SIG_CSV = resolve_path(Path(cfg["inputs"]["ap_deltas_csv"]))
    OUT_DIR = resolve_path(Path(cfg["output"]["charts_dir"]))
    PDF_OUT = resolve_path(Path(cfg["output"]["consolidated_pdf"]))
except Exception as e:
    # Fallback to defaults if config loading fails
    print(f"Warning: Could not load config, using defaults: {e}")
    SIG_CSV = "results/analysis/AP_composites_deltas_with_BHFDR.csv"
    OUT_DIR = Path("results/figures/charts_with_stars")
    PDF_OUT = "results/figures/Consolidated_Table_Plots.pdf"
