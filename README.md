...
│   ├── stage01_ingestion/  # Data loading and BookNLP I/O
│   ├── stage02_preprocessing/  # Text cleaning and tokenization
│   ├── stage03_modeling/   # BERTopic training and retraining
...
│   ├── stage07_analysis/   # Statistical analysis and visualization
...
For GPU acceleration with RAPIDS, ensure you have:
- CUDA 12.x installed
- Compatible GPU (NVIDIA with compute capability 7.0+)
- The RAPIDS packages in `requirements.txt` will install automatically
...
- **`data/raw/`**: Raw input data (excluded from git - see below)
  - EPUB files and full text novels
  - **Note**: These files are excluded from the repository due to size and copyright considerations
...
- Large file sizes (1.2GB+)
- Copyright restrictions on novel texts

To run the pipeline, you will need:

1. **Processed data files** (smaller, can be shared):
   - `data/processed/chapters.csv`
   - `data/processed/goodreads.csv`
   - `data/processed/custom_stoplist.txt`

2. **Raw data** (if starting from scratch):
   - EPUB files of romantic novels
   - Goodreads metadata and ratings
   - BookNLP processing outputs

**Contact the repository maintainers** for information on obtaining processed datasets or data preparation scripts.

### Data Exclusion Policy

The following are excluded from git via `.gitignore`:
- `data/raw/` - Raw novel texts and EPUBs
- `data/interim/` - Intermediate processing outputs
- Large output files (`.npz`, large `.csv`)
- Model files (`.pt`, `.pkl`, `.h5`)

This keeps the repository size manageable while preserving reproducibility through processed data files.
...
All paths and parameters are configured via YAML files. Paths are resolved relative to the project root using `src.common.config.resolve_path()`.

**Key configuration files:**
- `configs/paths.yaml` - All data and output paths
- `configs/bertopic.yaml` - BERTopic model settings
- `configs/octis.yaml` - Optimization search space
- `configs/selection.yaml` - Selection criteria (enforces min_nr_topics >= 200)

See [REPOSITORY_STRUCTURE.md](REPOSITORY_STRUCTURE.md) for detailed configuration documentation.
...
- Test individual stages before running the full pipeline
- Verify configuration changes don't break existing workflows
- Check that outputs match expected formats

### Documentation

- Update relevant documentation when adding features
- Add docstrings to new functions/classes
- Update `REPOSITORY_STRUCTURE.md` if structure changes
...
@software{romantic_novels_nlp,
  title = {Romantic Novels NLP Research Project},
...
- [REPOSITORY_STRUCTURE.md](REPOSITORY_STRUCTURE.md) - Detailed repository structure and file descriptions
- [reports/findings/](reports/findings/) - Research findings and analysis reports
- [mcp_research_planning/](mcp_research_planning/) - Research planning documents
